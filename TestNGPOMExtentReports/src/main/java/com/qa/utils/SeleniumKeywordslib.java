...
public class SeleniumKeywordslib  {
 ...
 * '################################################################################################
 */	
public String enterText(WebDriver webDriver,WebElement objName,String inputValue ,ExtentTest extentTestReport){
		
		FreeCRMLogger.writeToLog("Input","input()"," method called" );
		String status="false";
	    try {
	   WebElement txtField = objName;
		     ...
		     if (txtField.isDisplayed()) {
					...
					if (initText != null && initText.length() > 0) {
						...
						FreeCRMLogger.writeToLog("Input",	"input() ",	"after deleting the text");
					}
					...
					if(txtField.getAttribute("type").equalsIgnoreCase("password"))
					{
					     ...
					     byte[] encrypted = cipher.doFinal(inputValue.getBytes());
					       extentTestReport.log(LogStatus.PASS, " Enter " +objName +":  " +encrypted);
							FreeCRMLogger.writeToLog(objName+ " edit box successfully accepted the value ");
			...
			try{
				...
				FreeCRMLogger.writeToLog(objName+ " unable to enter value in textbox "+e.getMessage().split("\n"));
		        ...
		        extentTestReport.log(LogStatus.FAIL, extentTestReport.addScreenCapture(ExtentManager.CaptureScreen(webDriver, propertyFileReader.getValue("SNAPSHOT_FOLDER")+objName+": " +inputValue))); 
...
public String closeBrowser(WebDriver webDriver,ExtentReports extentReports,ExtentTest extentTestReport) {
	...
	FreeCRMLogger.writeToLog("SeleniumKeywordslib", "closeBrowser() ",	" method called");
	String status ="false";
	try {
		if (webDriver != null) {
			//webDriver.close();
		...
		return status;
		  ...
		  {
			  e.printStackTrace();
				FreeCRMLogger.writeToLog("SeleniumKeywordslib", "CloseBrowser() ",	"Exception thrown while closing browser");
				String err[]=e.getMessage().split("\n");
				status=  "Exception " +err[0].replaceAll("'", "") + " Occurred";
		...
		finally
		{
	        ...
	        extentTestReport.log(LogStatus.FAIL, extentTestReport.addScreenCapture(ExtentManager.CaptureScreen(webDriver, propertyFileReader.getValue("SNAPSHOT_FOLDER")))); 
	    	// extentReports.endTest(extentTestReport);
	    	// extentReports.flush();
	         //extentReports.close(); 
	        webDriver.close();
 ...
 * '################################################################################################
 */
public String click(WebDriver webDriver,WebElement objname,ExtentTest extentTestReport) {
	
	FreeCRMLogger.writeToLog("Click", " click() "," method called");
	String status = "false";
	try {
		  //List<WebElement> listElements= getElements(webDriver, objname,selector,selectorvalue);
		  ...
		  if (txtField.isDisplayed()) {
			...
			FreeCRMLogger.writeToLog("Click", " click() "," button clicked");
			
			
		  }else{
			  status = "false";	
			FreeCRMLogger.writeToLog("Click ", " click()"," Object not found");
			return status;
		}
		...
		{
			ex.printStackTrace();
			FreeCRMLogger.writeToLog("Click ", " click()"," Exception occurred");
			String err[]=ex.getMessage().split("\n");
			 status = "Exception " +err[0].replaceAll("'", "") + " Exception Occurred";
		}
		finally{
			extentTestReport.log(LogStatus.FAIL, "Unable to click "+objname);
	        extentTestReport.log(LogStatus.FAIL, extentTestReport.addScreenCapture(ExtentManager.CaptureScreen(webDriver, propertyFileReader.getValue("SNAPSHOT_FOLDER")+objname))); 
	    	// extentReports.endTest(extentTestReport);
	    	// extentReports.flush();
	         //extentReports.close(); 
	        webDriver.close();
...
public String selectWindow(WebDriver webDriver,ExtentReports extentReports,ExtentTest extentTestReport) {
	...
	FreeCRMLogger.writeToLog("select Window", " selectWindow() ","select Window method called");
	String status = "false";
	
	try {

		if (webDriver == null) {
			FreeCRMLogger.writeToLog("SeleniumKeywordslib",
		 ...
		 if (!availableWindows.isEmpty()) {
	      for(String winHandle : webDriver.getWindowHandles()){
	    	  FreeCRMLogger.writeToLog("SeleniumKeywordslib", "select Window ","No Of windows available" + webDriver.getWindowHandles().size());
	    	  webDriver.switchTo().window(winHandle);
	         	FreeCRMLogger.writeToLog("SeleniumKeywordslib","select Window() ","'"+ "' =>Document is activated successfully");
		...
		FreeCRMLogger.writeToLog("SeleniumKeywordslib","select Window() ","Available windows size is zero");
			status = "false";
			return winHandleBefore;
		}
		 ...
		 {
			    ex.printStackTrace();
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","select Window() ", "Exeption thrown");
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","select Window() ", "'" + "'object doesnot exists");
				String err[]=ex.getMessage().split("\n");
				 status = "Exception " +err[0].replaceAll("'", "") + " Exception Occurred";
				return status;
		 }
...
public boolean windowActivate(WebDriver webDriver, String title,ExtentReports extentReports,ExtentTest extentTestReport) {
	FreeCRMLogger.writeToLog("SeleniumKeywordslib", "windowActivate() ",
			"title - " + title);
	boolean status = false;
	try {

		if (webDriver == null) {
			FreeCRMLogger.writeToLog("SeleniumKeywordslib","windowActivate() ", "Browser is null");
			return false;
		}
    	Set<String> availableWindows = webDriver.getWindowHandles();
		FreeCRMLogger.writeToLog("SeleniumKeywordslib","windowActivate() ", "No Of windows available"+ availableWindows.size());
		FreeCRMLogger.writeToLog("SeleniumKeywordslib","windowActivate() ", "No Of windows available"+ webDriver.getWindowHandles().size());
		...
		if (!availableWindows.isEmpty()) {
			if (title.equals("ActivateLastWindow")) {
				for (Iterator i = availableWindows.iterator(); i.hasNext();) {
					...
					FreeCRMLogger.writeToLog("SeleniumKeywordslib","windowActivate() ","'"+ webDriver.switchTo().window(selectedWindowHandle).getTitle()+ "' =>Window is activated successfully");
					status = true;
					extentTestReport.log(LogStatus.PASS, " switch to child window ");
				}
				...
				for (String windowId : webDriver.getWindowHandles()) {
					...
					FreeCRMLogger.writeToLog("SeleniumKeywordslib","windowActivate() ", "in for title - "+ webDriver.switchTo().window(windowId).getTitle());
					if (webDriver.switchTo().window(windowId).getTitle().equals(title)) {
						FreeCRMLogger.writeToLog("SeleniumKeywordslib","windowActivate() ","'"+ title+ "' =>Document is activated successfully");
						status = true;
						return status;
					}
				}
			...
			FreeCRMLogger.writeToLog("SeleniumKeywordslib","windowActivate() ","Available windows size is zero");
			status = false;
			return status;
		}
		 ...
		 try
		 {
				...
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","select Window() ", "Exeption thrown");
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","select Window() ", "'" + title+ "'object doesnot exists");
				return false;
		 }
		 finally{
				extentTestReport.log(LogStatus.FAIL, "Unable to switch to window ");
 ...
 */
 ...
 public String mouseOver(WebDriver webDriver, String objName,String selector,String selectorvalue,ExtentReports extentReports,ExtentTest extentTestReport) {
        FreeCRMLogger.writeToLog("SeleniumKeywordslib", "mouseOver() "," object name" + objName);
        String status = "false";
        try {
               ...
               if (list.size() == 1) {
                     FreeCRMLogger.writeToLog("SeleniumKeywordslib", "mouseOver() ", "found more than one element.");
                     ...
                     if (element.isDisplayed()) {
                            ...
                            act.moveToElement(webDriver.findElement(By.xpath(submenu2path))).click(webDriver.findElement(By.xpath(submenu2path))).build().perform();
                            act.moveToElement(webDriver.findElement(By.xpath(submenu3path))).click(webDriver.findElement(By.xpath(submenu3path))).build().perform();
                            FreeCRMLogger.writeToLog("SeleniumKeywordslib", "mouseOver() ", "'" + objName + "' object was not visible");
                            status = "true";
                            extentTestReport.log(LogStatus.PASS, "click on Menu Item" +objName);
                            return status;
                     }
                     ...
                     FreeCRMLogger.writeToLog("SeleniumKeywordslib","mouseOver() ", "'" + objName+ "' object was not found");
                     status = "false";
                     extentTestReport.log(LogStatus.FAIL, "click on Menu Item" +objName);
                     FreeCRMLogger.writeToLog("SeleniumKeywordslib", "mouseOver() ", "object not found");
                     return status;
               }
        	...
        	try{
              ...
              // npiCTRMLogger.writeToLog("SeleniumKeywordslib", "mouseOver() ","Exception occurred");
                FreeCRMLogger.writeToLog("SeleniumKeywordslib", "mouseOver() ","Exception '" + ex.getMessage() + "' Occurred");
                String err[]=ex.getMessage().split("\n");
				status = "Exception " +err[0].replaceAll("'", "") + " Exception Occurred";
				return status;
...
public String selectFromList(WebDriver webDriver, String objName,String selector, String selectorvalue,String input, String selection,ExtentReports extentReports,ExtentTest extentTestReport) {
		FreeCRMLogger.writeToLog("SeleniumKeywordslib", "select FromList() "," objName - " + objName + ".. input - " + input+ ".. selection - " + selection);
		String status = "false";
		...
		try {
				...
				if (selectElement != null) {
					if (selectElement.isDisplayed()) {
						...
						if ((selectElement.getTagName().equalsIgnoreCase("input"))|| (selectElement.getTagName().equalsIgnoreCase("span"))) {
							FreeCRMLogger.writeToLog("SeleniumKeywordslib","select FromList()--- TagName",selectElement.getTagName());
							selectElement.clear();
							selectElement.sendKeys(input);
							selectElement.sendKeys(Keys.RETURN);
							FreeCRMLogger.writeToLog("SeleniumKeywordslib","select FromList() ",input+ "=> Item was selected successfully from the"+ "'" + objName + "'"+ "ListBox");
							status = "true";
							...
							if (selection.equalsIgnoreCase("value")) {
								FreeCRMLogger.writeToLog("SeleniumKeywordslib","select FromList() ","by option value");
								...
								FreeCRMLogger.writeToLog("SeleniumKeywordslib","select FromList() ", "by label");
								...
								FreeCRMLogger.writeToLog("SeleniumKeywordslib","select FromList() ","by option index");
							...
							FreeCRMLogger.writeToLog("SeleniumKeywordslib","select FromList()--- TagName is not input neither select",selectElement.getTagName());
							status = "false";
						...
						FreeCRMLogger.writeToLog("SeleniumKeywordslib","select FromList() ", "'" + objName+ "' list was not visible");
						status = "false";
						extentTestReport.log(LogStatus.FAIL, "select "+input +" option from " +objName);
						return status;
					}
				} else {
					FreeCRMLogger.writeToLog("SeleniumKeywordslib","select FromList() ","Invalid keyword usage. '" + objName+ "' is not a list");
					status = "false";
					extentTestReport.log(LogStatus.FAIL, "select "+input +" option from " +objName);
					return status;
				}
            ...
            try{
                ex.printStackTrace();
                String err[]=ex.getMessage().split("\n");
                FreeCRMLogger.writeToLog("Input", "input()", " Object not found exeption thrown");
                status = "Exception " +err[0].replaceAll("'", "") + " Occurred";
                return status;
            }
 ...
 */
 ...
 public String clickAndSelect(WebDriver webDriver, WebElement objName,String selector,String selectorvalue,String inputtxt,ExtentReports extentReports,ExtentTest extentTestReport) {
        FreeCRMLogger.writeToLog("SeleniumKeywordslib", "clickAndSelect"," object name" + objName);
        String status = "false";
        try {
        	  Actions act = new Actions(webDriver);
             ...
             //  List<WebElement> list = getElements(webDriver, objName, selector,selectorvalue);
               if (objName.isDisplayed()) {
                     FreeCRMLogger.writeToLog("SeleniumKeywordslib","clickAndSelect() ", "found more than one element.");
                     ...
                     if (element.isDisplayed()) {
                            ...
                            FreeCRMLogger.writeToLog("SeleniumKeywordslib","clickAndSelect() ", "'" + objName+ "' =>Hover is performed sucessfully ");
                            return status;
                     } else {
                            act.moveToElement(element).click(element).build().perform();
                            act.moveToElement(webDriver.findElement(By.xpath(path))).click(webDriver.findElement(By.xpath(path))).build().perform();
                            
                            FreeCRMLogger.writeToLog("SeleniumKeywordslib", "clickAndSelect() ", "'" + objName + "' object was not visible");
                            status = "true";
                            extentTestReport.log(LogStatus.PASS, "select "  +inputtxt +" from "+objName+ " Menu List");
                            return status;
                     }
               ...
               ns.printStackTrace();
               FreeCRMLogger.writeToLog("SeleniumKeywordslib", "clickAndSelect() ","Object not found exeption thrown");
               FreeCRMLogger.writeToLog("SeleniumKeywordslib", "clickAndSelect() ", "'" + objName + "'object does not exist");
               status = "false";
        	...
        	{
               ex.printStackTrace();
               FreeCRMLogger.writeToLog("SeleniumKeywordslib", "clickAndSelect() ","Exception occurred");
               extentTestReport.log(LogStatus.FAIL, "select "  +inputtxt +" from "+objName+ "Menu List");
               String err[]=ex.getMessage().split("\n");
			   status = "Exception " +err[0].replaceAll("'", "") + " Exception Occurred";
				return status;
  ...
  public String getText(WebDriver webDriver, String objName,String selector,String selectorvalue,ExtentReports extentReports,ExtentTest extentTestReport) {
		FreeCRMLogger.writeToLog("SeleniumKeywordslib","getText() objName - ", objName + ".. selector - "+ selector);
		String text = null;
		try {
		...
		//	String msg[];
			if (list.size() > 1) {
				FreeCRMLogger.writeToLog("SeleniumKeywordslib", "getText()",list.size() + " number of elements Found");
				...
				if (txtField != null) {
					...
					if (text.equals("") || (text == null)) {
						...
						//		 msg=text.split("\\n");
								...
								{
									FreeCRMLogger.writeToLog("SeleniumKeywordslib","getText()","'"+ text+ "' =>value was successfully captured from edit box=: "+ objName);
						...
						FreeCRMLogger.writeToLog("SeleniumKeywordslib","getText()", "'" + objName+ "' =>does not visible!");
			...
			ns.printStackTrace();
			FreeCRMLogger.writeToLog("SeleniumKeywordslib", "getText()","Object not found exeption thrown");
			FreeCRMLogger.writeToLog("SeleniumKeywordslib", "getText()", "'"+ objName + "'object does not exist");
			...
			{
			ex.printStackTrace();
			FreeCRMLogger.writeToLog("SeleniumKeywordslib", "getText()","Exception occurred");
   ...
   */
   ...
   public String getText(WebDriver webDriver, String objName,String selector,String selectorvalue, String propName,ExtentReports extentReports,ExtentTest extentTestReport) {
 		...
 		try {
 			...
 			List<WebElement> list = getElements(webDriver, objName, selector,selectorvalue);
 				...
 				FreeCRMLogger.writeToLog("SeleniumKeywordslib", "getText()","found " + list.size() + "elements");
 			if (list.size() > 0) {
   ...
   public String captureScreenshot(WebDriver webDriver,String name,ExtentReports extentReports,ExtentTest extentTestReport) {
		...
		FreeCRMLogger.writeToLog("SeleniumKeywordslib", "captureScreenshot() ","snapShotPath - " + snapShotPath);
		//File sfolder = new File(snapShotPath);
		SimpleDateFormat df = new SimpleDateFormat("dd-MM-yyyy HH-mm-ss.SSS");
    	//String fname = snapShotPath + "CaptureScreen" + "_"+ df.format(new Date()) +"_"+name+ ".png";
		try {

			FreeCRMLogger.writeToLog("SeleniumKeywordslib", "captureScreenshot() ","Getting generic screenshot");
//			File source = ((TakesScreenshot) webDriver).getScreenshotAs(OutputType.FILE);
//			if (sfolder.exists() == false) {
//				sfolder.mkdirs();
//			}
//			FileUtils.copyFile(source, new File(fname));
			FreeCRMLogger.writeToLog("SeleniumKeywordslib", "captureScreenshot() ","After getting generic screenshot");
			status = "true";
			extentTestReport.log(LogStatus.PASS, extentTestReport.addScreenCapture(ExtentManager.CaptureScreen(webDriver, propertyFileReader.getValue("SNAPSHOT_FOLDER")+name+"_"+ df.format(new Date()))));
			return status;
           ...
           {
        	ex.printStackTrace();
   			FreeCRMLogger.writeToLog("SeleniumKeywordslib", "captureScreenshot() ",ex.getMessage());
   			String err[]=ex.getMessage().split("\n");
			 status = "Exception " +err[0].replaceAll("'", "") + " Exception Occurred";
			return status;
           }
			finally
			{
		         ...
		         extentTestReport.log(LogStatus.FAIL, extentTestReport.addScreenCapture(ExtentManager.CaptureScreen(webDriver, propertyFileReader.getValue("SNAPSHOT_FOLDER")+name+"_"+ df.format(new Date())))); 
		    	// extentReports.endTest(extentTestReport);
		    	// extentReports.flush();
		         //extentReports.close(); 
		        webDriver.close();
   ...
   public String captureScreenshot(WebDriver webDriver,ExtentReports extentReports,ExtentTest extentTestReport) {
		...
		try {
			...
			FreeCRMLogger.writeToLog("SeleniumKeywordslib", "captureScreenshot() ","After getting generic screenshot");
			status = "true";
			extentTestReport.log(LogStatus.PASS, extentTestReport.addScreenCapture(ExtentManager.CaptureScreen(webDriver, propertyFileReader.getValue("SNAPSHOT_FOLDER")+"")));
			return status;
    ...
    */
    public String switchtochildwindow(WebDriver webDriver,ExtentReports extentReports,ExtentTest extentTestReport)
     {
    	  ...
    	  try
    	  {
    		...
    		FreeCRMLogger.writeToLog("SeleniumKeywordslib", "switchtochildwindow() ","switchtochildwindow");
    		...
    		//webDriver.switchTo().window(ge)
    		 return firstWinHandle;
    		  ...
    		  {
    			  ex.printStackTrace();
    	  			FreeCRMLogger.writeToLog("SeleniumKeywordslib", "switchtochildwindow() ",ex.getMessage());
    	  			String err[]=ex.getMessage().split("\n");
    				 status = "Exception " +err[0].replaceAll("'", "") + " Exception Occurred";
    				return status;
    		  }
     ...
     */
     ...
     public void stopExecution(WebDriver webDriver,String browser) {
  		...
  		try {
  			...
  			e.printStackTrace();
  	  		FreeCRMLogger.writeToLog("SeleniumKeywordslib", "stopExecution","Exception while ending Session" + e.getMessage());
  	  		e.printStackTrace();
  			FreeCRMLogger.writeToLog("SeleniumKeywordslib", "stopExecution","Exception while ending Session" + e.getMessage());
  			//webDriver.close();
  			//webDriver.quit();
	...
	public String launchUrl(WebDriver webDriver, String url, ExtentReports extentReports,ExtentTest extentTestReport) {
		...
		try {
			...
			if (ex.getMessage().toString().contains("Modal dialog present")) {
				...
				FreeCRMLogger.writeToLog("LaunchUrl", "launchUrl() ",	" not able to launch.");
				String err[]=ex.getMessage().split("\n");
	...
	public String clickByJavaScript(WebDriver webDriver, String objName,String selector,String selectorvalue, ExtentReports extentReports,ExtentTest extentTestReport) {
		...
		//	waitForExistence(webDriver, objName, selector,selectorvalue);
			...
			FreeCRMLogger.writeToLog("SeleniumKeywordslib","clickByJavaScript() ", "'" + objName+ "' element does not exist");
			status = "false";
            ...
            FreeCRMLogger.writeToLog("SeleniumKeywordslib","clickByJavaScript() ","web elements size - " + list.size());
			if (list.size() > 0) {
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","clickByJavaScript() ","found more than one element");
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","clickByJavaScript() ", "clicking 1st element");
			//	for (WebElement element : list) {
				WebElement element = list.get(0);
					if ((element.isDisplayed())||(!element.isDisplayed())) {
						FreeCRMLogger.writeToLog("SeleniumKeywordslib","clickByJavaScript() ", "object visible");
						JavascriptExecutor js = (JavascriptExecutor) webDriver;
			...
			try{
			ex.printStackTrace();
		    FreeCRMLogger.writeToLog("SeleniumKeywordslib", "clickByJavaScript() ","Exception '" + ex.getMessage() + "' Occurred");
		    String err[]=ex.getMessage().split("\n");
			status = "Exception " +err[0].replaceAll("'", "") + " Exception Occurred";
			return status;
			}
	...
	public String clickByJavaScriptByText(WebDriver webDriver, String objName,String selector,String selectorvalue,String text, ExtentReports extentReports,ExtentTest extentTestReport) {
		...
		//	waitForExistence(webDriver, objName, selector,selectorvalue);
            ...
            FreeCRMLogger.writeToLog("SeleniumKeywordslib","clickByJavaScript() ","web elements siOze - " + list.size());
			if (list.size() > 0) {
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","clickByJavaScript() ","found more than one element");
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","clickByJavaScript() ", "clicking 1st element");
			//	for (WebElement element : list) {
	...
	public String verifyText(WebDriver webDriver, WebElement objName,String selector, String selectorvalue,String inputTxt, String matchType,ExtentReports extentReports,ExtentTest extentTestReport) {
		FreeCRMLogger.writeToLog("SeleniumKeywordslib", "verifyText() "," executing verifyText objName - " + objName+ ".. inputTxt - " + inputTxt + ".. matchType - "+ matchType);
		String status = "false";
		String text = null;
		...
		try {
		  ...
		  // waitForExistence(webDriver, objName, selector,selectorvalue);
		 //  List<WebElement> list = getElements(webDriver, objName, selector,selectorvalue);
			FreeCRMLogger.writeToLog("SeleniumKeywordslib", "verifyText() ","web elements size - " );
			if (txtField.isDisplayed()) {
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","verifyText() ", "found more than one element");
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","verifyText() ", "verifying 1st text field");
				//for (WebElement txtField : list) {
					if (txtField.getTagName().equalsIgnoreCase("INPUT")&& txtField.getAttribute("type") != null&& txtField.getAttribute("type").equalsIgnoreCase("text")) {
						...
						if (txtField.isDisplayed()) {
							...
							FreeCRMLogger.writeToLog("SeleniumKeywordslib","verifyText() ", "Text - " + text);
							if (matchType.equalsIgnoreCase("Exact")) {
								if (inputTxt.trim().equalsIgnoreCase(text.trim())) {
									FreeCRMLogger.writeToLog("SeleniumKeywordslib","verifyText() ","'"+ text+ "' =>value matches input text  "+ inputTxt);
									status = "true";
									extentTestReport.log(LogStatus.PASS, objName +" matches with " +inputTxt);
									return status;
								} else {
									FreeCRMLogger.writeToLog("SeleniumKeywordslib","verifyText() ","'"+ text+ "' =>value doesnt match input text  "+ inputTxt);
									status = "false";
								...
								if (inputTxt.equalsIgnoreCase(text)) {
									FreeCRMLogger.writeToLog("SeleniumKeywordslib","verifyText() ", "'" + text+ "' =>Object Exists  "+ inputTxt);
									status = "false";
									extentTestReport.log(LogStatus.FAIL, objName +" matches with " +inputTxt);
									return status;
								} else {
									FreeCRMLogger.writeToLog("SeleniumKeywordslib","verifyText() ","'"+ text+ "' =>Object doesn't exist  "+ inputTxt);
									status = "true";
							...
							FreeCRMLogger.writeToLog("SeleniumKeywordslib","verifyText() ", "'" + objName+ "text field  was not visible");
							status = "false";
							return status;
						}
					} else {
						FreeCRMLogger.writeToLog("SeleniumKeywordslib","verifyText() ", "'" + objName+ "' is not a text field");
						if (txtField.isDisplayed()) {
							text = txtField.getText();
							upToNCharacters = text.substring(0, Math.min(text.length(), 50));
							FreeCRMLogger.writeToLog("SeleniumKeywordslib","verifyText() ", "Text - " + text);
							...
							if (matchType.equalsIgnoreCase("Exact")) {
								...
								if (text.trim().contains(inputTxt.trim())||(inputTxt.trim().contains(text.trim()))) {
									FreeCRMLogger.writeToLog("SeleniumKeywordslib","verifyText() ","'"+ text+ "' =>Object Text contains  "+ inputTxt);
									status = "true";
									extentTestReport.log(LogStatus.PASS, objName +" contains " +upToNCharacters);
									return status;
								} else {
									FreeCRMLogger.writeToLog("SeleniumKeywordslib","verifyText() ","'"+ text+ "' =>Object Text doesnt contain  "+ inputTxt);
									status = "false";
	...
	//}
            ...
            try{
                ex.printStackTrace();
                String err[]=ex.getMessage().split("\n");
                FreeCRMLogger.writeToLog("Input", "input()", " Text not found exeption thrown");
                status = "Exception " +err[0].replaceAll("'", "") + " Occurred";
                return status;
            }
	...
	public String selectCheckbox(WebDriver webDriver, WebElement objName,String checkboxData,ExtentReports extentReports,ExtentTest extentTestReport) {
		...
		try {
			...
			FreeCRMLogger.writeToLog("SeleniumKeywordslib", "selectCheckbox() ","web elements size - " );			
			if (element.isDisplayed()) {

					if (selValue.equalsIgnoreCase("true")) {
						if (element.isSelected()) {
							extentTestReport.log(LogStatus.PASS, "click on "+paramvalues[i]+" Checkbox");
							FreeCRMLogger.writeToLog("SeleniumKeywordslib","selectCheckbox() ", "'" + objName+ "' => Checkbox is already selected");
							status ="true";
						} else {
							element.click();
							extentTestReport.log(LogStatus.PASS, "click on "+paramvalues[i]+" Checkbox");
							FreeCRMLogger.writeToLog("SeleniumKeywordslib","selectCheckbox() ", "'" + objName+ "' => Checkbox is  checked");
							status ="true";
						}
					} else {
						if (element.isSelected()) {
							element.click();
							extentTestReport.log(LogStatus.PASS, "click on "+paramvalues[i]+" Checkbox");
							FreeCRMLogger.writeToLog("SeleniumKeywordslib","selectCheckbox() ", "'" + objName+ "' => Checkbox is unchecked");
							status ="true";
						} else {
							extentTestReport.log(LogStatus.PASS, "'+element+'" +"Checkbox is already selected");
							FreeCRMLogger.writeToLog("SeleniumKeywordslib","selectCheckbox() ", "'" + objName+ "' => Checkbox is already unchecked");
							status ="true";
						}
					...
					FreeCRMLogger.writeToLog("SeleniumKeywordslib","selectCheckbox() ", "'" + objName+ "CheckBox was not visible");
			...
			if(paramvalues.length==0)
			{
				...
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","selectCheckbox() ", "'" + objName+ " CheckBox was not found");
            ...
            ns.printStackTrace();
            FreeCRMLogger.writeToLog("SeleniumKeywordslib", "selectCheckbox() ","Object not found exeption thrown");
            FreeCRMLogger.writeToLog("SeleniumKeywordslib", "selectCheckbox() ", "'" + objName + "'object does not exist");
            status = "false";
     	...
     	{
            ex.printStackTrace();
            FreeCRMLogger.writeToLog("SeleniumKeywordslib", "selectCheckbox() ","Exception occurred");
	 ...
	 */
			...
			public String selectRadioButton(WebDriver webDriver, String objName,String selector,String selectorvalue,ExtentReports extentReports,ExtentTest extentTestReport) {
				...
				try {
				    waitForExistence(webDriver, objName, selector,selectorvalue);
					List<WebElement> list = getElements(webDriver, objName, selector,selectorvalue);
					FreeCRMLogger.writeToLog("SeleniumKeywordslib","selectRadioButton()","web elements size - " + list.size());
					if (list.size() > 1) {
						FreeCRMLogger.writeToLog("SeleniumKeywordslib","selectRadioButton()","found more than one element");
						FreeCRMLogger.writeToLog("SeleniumKeywordslib","selectRadioButton()", "clicking 1st element");
						for (WebElement element : list) {
							if ((element.isDisplayed()) && (!element.isSelected())){
								webDriver.manage().timeouts().pageLoadTimeout(60, TimeUnit.SECONDS);
									...
									FreeCRMLogger.writeToLog("SeleniumKeywordslib","selectRadioButton()", "after click");
									FreeCRMLogger.writeToLog("SeleniumKeywordslib","selectRadioButton()","'"+ objName + "' => Radio Button was successfully clicked");
								...
								webDriver.manage().timeouts().pageLoadTimeout(60, TimeUnit.SECONDS);
									...
									FreeCRMLogger.writeToLog("SeleniumKeywordslib","selectRadioButton()","'"+ objName + "' Radio Button was not visible_______");
						...
						FreeCRMLogger.writeToLog("SeleniumKeywordslib","selectRadioButton()", "clicking element");
						WebElement element = list.get(0);
						...
						if (element != null) {
							if (element.isDisplayed()) {
								...
								webDriver.manage().timeouts().pageLoadTimeout(60, TimeUnit.SECONDS);
								status = "true";
								FreeCRMLogger.writeToLog("SeleniumKeywordslib","selectRadioButton()","'"+ objName+ "' => Radio Button was successfully selected");
								FreeCRMLogger.writeToLog("SeleniumKeywordslib","selectRadioButton()", "after click");
								extentTestReport.log(LogStatus.PASS, "click on " +objName +"radio button");
								return status;
							} else {
								FreeCRMLogger.writeToLog("SeleniumKeywordslib","selectRadioButton()","'"+ objName + "' Radio Button was not visible******");
								element.sendKeys(Keys.ENTER);
								status = "true";
								extentTestReport.log(LogStatus.FAIL, objName +" radio button is not found");
								return status;
							...
							FreeCRMLogger.writeToLog("SeleniumKeywordslib","selectRadioButton()", "'" + objName + "' Radio Button was not found");
							status = "Radio Button was not found";
							extentTestReport.log(LogStatus.FAIL, objName +" radio button is not found");
							return status;
						}
					} else {
						FreeCRMLogger.writeToLog("SeleniumKeywordslib","selectRadioButton()", "'" + objName+ "' Radio Button was not selected");
					...
					{
						ex.printStackTrace();
						FreeCRMLogger.writeToLog("SeleniumKeywordslib","selectRadioButton()", "Exception occurred");
						FreeCRMLogger.writeToLog("SeleniumKeywordslib","selectRadioButton()", "Exception '" + ex.getMessage()+ "' Occurred");
						String err[]=ex.getMessage().split("\n");
						status =  "Exception " +err[0].replaceAll("'", "") + " Occurred";
						return status;
					}
		...
		public String unSelectCheckbox(WebDriver webDriver, WebElement objName,String selectorvalue,ExtentReports extentReports,ExtentTest extentTestReport) {
			...
			try {
				...
				FreeCRMLogger.writeToLog("SeleniumKeywordslib", "unSelectCheckbox() ","web elements size - ");
					...
					if (element.isDisplayed()) {
						...
						if (selValue.equalsIgnoreCase("true")) {
							if (element.isSelected()) {
								extentTestReport.log(LogStatus.PASS, "click on "+objName +"checkbox");
								FreeCRMLogger.writeToLog("SeleniumKeywordslib","unSelectCheckbox() ", "'" + objName+ "' => checkbox is already selected");
								return "true";
							} else {
								element.click();
								extentTestReport.log(LogStatus.PASS, "click on "+objName +"checkbox");
								FreeCRMLogger.writeToLog("SeleniumKeywordslib","unSelectCheckbox() ", "'" + objName+ "' =>checkbox  is  checked");
								return "true";
							}
						} else {
							if (element.isSelected()) {
								element.click();
								extentTestReport.log(LogStatus.PASS, "click on "+objName +"checkbox");
								FreeCRMLogger.writeToLog("SeleniumKeywordslib","unSelectCheckbox() ", "'" + objName+ "' =>checkbox is unchecked");
								return "true";
							} else {
								extentTestReport.log(LogStatus.PASS, "'+objName+'" +"checkbox is already selected");
								FreeCRMLogger.writeToLog("SeleniumKeywordslib","unSelectCheckbox() ", "'" + objName+ "' => checkbox is already unchecked");
								return "true";
							}
						...
						extentTestReport.log(LogStatus.FAIL, "click on "+objName +"checkbox");
						FreeCRMLogger.writeToLog("SeleniumKeywordslib","unSelectCheckbox() ", "'" + objName+ "checkBox was not visible");
						status = "CheckBox was not visible";
						return status;
					}
				...
				else if(dropList.size()==1)
				{
					...
					extentTestReport.log(LogStatus.FAIL, "click on "+objName +"checkbox");
					FreeCRMLogger.writeToLog("SeleniumKeywordslib","unSelectCheckbox() ", "'" + objName+ " checkBox was not found");
					status = "CheckBox was not found";
					return status;
				}
	     	...
	     	{
	            ex.printStackTrace();
	            FreeCRMLogger.writeToLog("SeleniumKeywordslib", "unSelectCheckbox() ","Exception occurred");
	            extentTestReport.log(LogStatus.FAIL,"select " +objName+ "checkbox");
	            String err[]=ex.getMessage().split("\n");
				status = "Exception " +err[0].replaceAll("'", "") + " Exception Occurred";
				return status;
 ...
 */
 ...
 public String clickByText(WebDriver webDriver, String objName,String selector,String selectorvalue,String text,ExtentReports extentReports,ExtentTest extentTestReport) {
        FreeCRMLogger.writeToLog("SeleniumKeywordslib", "clickByText() "," object name" + objName);
        String status = "false";
        try {
               ...
               if (list.size() == 1) {
                     FreeCRMLogger.writeToLog("SeleniumKeywordslib", "clickByText() ", "found more than one element.");
                     Actions act = new Actions(webDriver);
                     WebElement element = list.get(0);
                     ...
                     if (element.isDisplayed()) {
                            ...
                            FreeCRMLogger.writeToLog("SeleniumKeywordslib", "clickByText() ", "'" + objName + "' object was not visible");
                            status = "true";
                            extentTestReport.log(LogStatus.PASS, "click on " +objName);
                            return status;
                     }
                     ...
                     FreeCRMLogger.writeToLog("SeleniumKeywordslib","clickByText() ", "'" + objName+ "' object was not found");
                     status = "false";
                     extentTestReport.log(LogStatus.FAIL, "click on Menu Item" +objName);
                     FreeCRMLogger.writeToLog("SeleniumKeywordslib", "clickByText() ", "object not found");
                     return status;
               }
        	...
        	try{
                ex.printStackTrace();
              // npiCTRMLogger.writeToLog("SeleniumKeywordslib", "mouseOver() ","Exception occurred");
                FreeCRMLogger.writeToLog("SeleniumKeywordslib", "clickByText() ","Exception '" + ex.getMessage() + "' Occurred");
                String err[]=ex.getMessage().split("\n");
				status = "Exception " +err[0].replaceAll("'", "") + " Exception Occurred";
				return status;
  ...
  */
  ...
  public String displayValueInGUI(WebDriver webDriver, String objName,ExtentReports extentReports,ExtentTest extentTestReport) {
         FreeCRMLogger.writeToLog("SeleniumKeywordslib", "displayValueInGUI() "," object name" + objName);
        String status = "";
         try {
                
         	                 FreeCRMLogger.writeToLog("SeleniumKeywordslib", "displayValueInGUI() ", "'" + objName + "' object was visible");
                             status = "true";
         	...
         	try{
                 ex.printStackTrace();
               // npiCTRMLogger.writeToLog("SeleniumKeywordslib", "mouseOver() ","Exception occurred");
                 FreeCRMLogger.writeToLog("SeleniumKeywordslib", "displayValueInGUI() ","Exception '" + ex.getMessage() + "' Occurred");
                 String err[]=ex.getMessage().split("\n");
 				status = "Exception " +err[0].replaceAll("'", "") + " Exception Occurred";
 				return status;
         		 ...
         		 extentTestReport.addScreenCapture(propertyFileReader.getValue("REPORT_PATH")+"objName");
             	 extentTestReport.log(LogStatus.FAIL, extentTestReport.addScreenCapture(ExtentManager.CaptureScreenForExcel(webDriver, propertyFileReader.getValue("FILE_DOWNLOAD_LOC")+"objName"))); 
             	// extentReports.endTest(extentTestReport);
             	// extentReports.flush();
             	 //extentReports.close(); 
         	}
  ...
  public boolean elementDisplayed(WebDriver webDriver,String objName,String selector,String selectorvalue, ExtentReports extentReports,ExtentTest extentTestReport) {
  	FreeCRMLogger.writeToLog("SeleniumKeywordslib", "elementDisplayed() ","elementDisplayed - ");
  	boolean status = false;
  	try {

  		if (webDriver == null) {
  			FreeCRMLogger.writeToLog("SeleniumKeywordslib","elementDisplayed() ", "Browser is null");
  			return false;
  		}
  		List<WebElement> list = getElements(webDriver, objName, selector,selectorvalue);
		
       if (list.size() > 0) {
		FreeCRMLogger.writeToLog("SeleniumKeywordslib","elementDisplayed()", list.size()+ " number of elements Found");
  		...
  		if(we.isDisplayed())
  		{
  			...
  			{
  				FreeCRMLogger.writeToLog("SeleniumKeywordslib", "elementDisplayed() ",we +"is displayed ");
				status = true;
  			...
  			{
  			  FreeCRMLogger.writeToLog("SeleniumKeywordslib", "elementDisplayed() ",objName +"is displayed ");
				status = false;
  		...
  		{
  			 FreeCRMLogger.writeToLog("SeleniumKeywordslib", "elementDisplayed() ",we +"is not displayed ");
				status = false;
  		...
  		if(element.isDisplayed()) 
	        {
  			...
  			{
  				FreeCRMLogger.writeToLog("SeleniumKeywordslib", "elementDisplayed() ",element +"is displayed ");
				status = true;
	 ...
	 {
		FreeCRMLogger.writeToLog("SeleniumKeywordslib","elementDisplayed() ","element is  not displayed");
		status = false;
		return status;
	 }
  	}
  	else
  	{
		...
		return status;
  		 ...
  		 {
  			    exception.printStackTrace();
  				FreeCRMLogger.writeToLog("SeleniumKeywordslib","elementDisplayed() ", "Exeption thrown");
  				FreeCRMLogger.writeToLog("SeleniumKeywordslib","elementDisplayed() ", "object doesnot exists");
  				return false;
  		 }
  		 finally{
  ...
  public boolean verifyMultileElementsVisible(WebDriver webDriver,String objName,String selector,String selectorValue,String elements, ExtentReports extentReports,ExtentTest extentTestReport) {
  	...
  	else if (list.size() == 1)
  	{
		...
		FreeCRMLogger.writeToLog("SeleniumKeywordslib","elementDisplayed()", list.size()+ " number of elements Found");
  		...
  		if(ele.isDisplayed()) 
	        {
  				FreeCRMLogger.writeToLog("SeleniumKeywordslib", "elementDisplayed() ",ele +"is displayed ");
				status = true;
				extentTestReport.log(LogStatus.FAIL, " Warning in Application");
				//return status;
    ...
    public boolean elementPresence(WebDriver webDriver,String objName,String selector,String selectorvalue, ExtentReports extentReports,ExtentTest extentTestReport) {
    	FreeCRMLogger.writeToLog("SeleniumKeywordslib", "elementPresence() ","elementPresence - ");
    	boolean status = false;
    	try {

    		if (webDriver == null) {
    			FreeCRMLogger.writeToLog("SeleniumKeywordslib","elementPresence() ", "Browser is null");
    			return false;
    		}
    		List<WebElement> list = getElements(webDriver, objName, selector,selectorvalue);
  		
         if (list.size() > 0) {
  		FreeCRMLogger.writeToLog("SeleniumKeywordslib","elementPresence()", list.size()+ " number of elements Found");

  		  WebElement we = list.get(0);
    		
    					FreeCRMLogger.writeToLog("SeleniumKeywordslib", "windowActivate() ",we +"is displayed ");
    					status = true;
    		...
    		if(element.isDisplayed()) 
  	        {
         		...
         		FreeCRMLogger.writeToLog("SeleniumKeywordslib", "elementPresence() ","element is displayed ");
  				status = true;
  	 ...
  	 {
  		FreeCRMLogger.writeToLog("SeleniumKeywordslib","elementPresence() ","element is  not displayed");
  		status = false;
  		return status;
  	 }
    	}
    	else
    	{
  		...
  		return status;
    		 ...
    		 {
    			    exception.printStackTrace();
    				FreeCRMLogger.writeToLog("SeleniumKeywordslib","elementPresence() ", "Exeption thrown");
    				FreeCRMLogger.writeToLog("SeleniumKeywordslib","elementPresence() ", "object doesnot exists");
    				return false;
    		 }
    		 finally{
    				extentTestReport.log(LogStatus.FAIL, "Unable to switch to window ");
  ...
  /*
	...
	public String setCheckbox(WebDriver webDriver, String objName,String selector,String selectorvalue,ExtentReports extentReports,ExtentTest extentTestReport) {
		...
		try {
			...
			FreeCRMLogger.writeToLog("SeleniumKeywordslib", "setCheckbox() ","web elements size - " + list.size());
			if (list.size() > 1) {
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","setCheckbox() ", "found more than one element");
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","setCheckbox() ", "clicking 1st element");
				WebElement element=list.get(0);
				//for (WebElement element : list) {
					if (element.isDisplayed()) {
						
							if (element.isSelected()) {
								FreeCRMLogger.writeToLog("SeleniumKeywordslib","setCheckbox() ", "'" + objName+ "' => is already selected");
								extentTestReport.log(LogStatus.PASS, "File Name " +objName);
								return "true";
							} else {
								element.click();
								FreeCRMLogger.writeToLog("SeleniumKeywordslib","setCheckbox() ", "'" + objName+ "' => is  checked");
								extentTestReport.log(LogStatus.PASS, "File Name " +objName);
								return "true";
					}
					}
					else {
						FreeCRMLogger.writeToLog("SeleniumKeywordslib","setCheckbox() ", "'" + objName+ "CheckBox was not visible");
						status = "CheckBox was not visible";
						return status;
					}
	...
	public boolean handleAlertDialog(WebDriver webDriver,ExtentReports extentReports,ExtentTest extentTestReport) {
		try {
			...
			FreeCRMLogger.writeToLog("SeleniumKeywordslib","handleAlertDialog() ", "ALERT Handled");
	  		 ...
	  		 {
	  			    exception.printStackTrace();
	  				FreeCRMLogger.writeToLog("SeleniumKeywordslib","handleAlertDialog() ", "ALERT not Handled");
	  		 }
	  		 finally{
	...
	public boolean handleAlertDialog(WebDriver webDriver, String val,ExtentReports extentReports,ExtentTest extentTestReport) {
		FreeCRMLogger.writeToLog("SeleniumKeywordslib","handleAlertDialog() ", "Value" + val);
		try {
	...
	public boolean compareData(WebDriver webDriver,double Actualvalue, String Expectedvalue,String Match,ExtentReports extentReports,ExtentTest extentTestReport) {
		FreeCRMLogger.writeToLog("SeleniumKeywordslib", "compareData() ","Actualvalue " + Actualvalue + " Expectedvalue "+ Expectedvalue + " Match " + Match);
		...
		try
		{
    		if (Match.equalsIgnoreCase("Exact")) {
				if ((Double.toString(Actualvalue)).equals(Expectedvalue)) {
					...
					FreeCRMLogger.writeToLog("SeleniumKeywordslib", "compareData() ","Actual data matches with expected data");
					return true;
				} else {
					extentTestReport.log(LogStatus.FAIL, "Actual datadoesn't  matches with expected data");
					FreeCRMLogger.writeToLog("SeleniumKeywordslib", "compareData() ","Actual doesn't match with expected data");
					return false;
				}
			} 
 				...
 				if ((Double.toString(Actualvalue)).contains(Expectedval)) {
					...
					FreeCRMLogger.writeToLog("SeleniumKeywordslib", "compareData() ","Actual data contains  expected data");
					return true;
				} else {
					extentTestReport.log(LogStatus.FAIL, "Actual doesn't contains expected data");
					FreeCRMLogger.writeToLog("SeleniumKeywordslib", "compareData() ","Actual doesn't contains expected data");
					return false;
				}
    			}
	  		 ...
	  		 {
	  			    exception.printStackTrace();
	  				FreeCRMLogger.writeToLog("SeleniumKeywordslib","compareData() ", "Unable to compare data");
	  		 }
	  		 finally{
	 ...
	 public boolean scrollPage(WebDriver webDriver, String objName,String selector,String selectorvalue,String text,ExtentReports extentReports,ExtentTest extentTestReport) {
			...
			try {
				...
				FreeCRMLogger.writeToLog("Scrolled to " +objName);
				status = true;
				return status;
			} catch (NoSuchElementException ns) {
				try
				{
				ns.printStackTrace();
				FreeCRMLogger.writeToLog("SeleniumKeywordslib", "scrollPage()","Scrolling Failed");
				return status;
			} 
			finally {
       		 extentTestReport.addScreenCapture(propertyFileReader.getValue("REPORT_PATH")+"objName");
	 ...
	 public String getCellDataForAnnouncementsTable(WebDriver webDriver, String objName,String selector,String selectorvalue, String rowName, String colName,ExtentReports extentReports,ExtentTest extentTestReport) {
			FreeCRMLogger.writeToLog("SeleniumKeywordslib", "getCellData()","getCellData  rowNo " + rowName + " colNo " + colName+ "..objName - " + objName);
			String value = null;
			String getVal = null;
			int rowNo = 0;
			int colNo = 0;
			...
			try {
				...
				if (we != null) {
                   ...
                   FreeCRMLogger.writeToLog("SeleniumKeywordslib","getRowByContent() ", "rows - " + rows.size()+colNo);
					if (rows != null) {
						int tempRowNo = -1;
					...
					FreeCRMLogger.writeToLog("SeleniumKeywordslib","getCellData()", rowName + "X" + colName);
					value1 = werowCell.getText();
					
					FreeCRMLogger.writeToLog("SeleniumKeywordslib","getRowByContent() ", "VALUE - "+ value1);

					if (value1 != null) {
						if (value1.contains(rowName)) {
							rowNo = tempRowNo;
							FreeCRMLogger.writeToLog("SeleniumKeywordslib","getRowByContent() ","VALUES EQUAL");
							FreeCRMLogger.writeToLog("SeleniumKeywordslib","getRowByContent() ","Row number captured is = \""+ rowNo + "\"");
							row = rowNo;
							List<WebElement> cols = null;
							cols = (we.findElements(By.xpath("//*[@class='k-grid-table']//tr//td[6]")));

							FreeCRMLogger.writeToLog("SeleniumKeywordslib","getRowByContent() ", "rows - " + cols.size());
							if (cols != null) {
								int tempColNo = -1;
							...
							FreeCRMLogger.writeToLog("SeleniumKeywordslib","getCellData()", rowName + "X" + colName +tempColNo);
							value2 = wecolCell.getText();
							FreeCRMLogger.writeToLog("SeleniumKeywordslib","getRowByContent() ", "VALUE - "+ value2);

							if (value2 != null) {
								  	extentTestReport.log(LogStatus.PASS," Value for  :  "+ colName + "   in RowName : " + rowName + " : is "+ value);
											FreeCRMLogger.writeToLog("SeleniumKeywordslib","getCellData()", "'" + objName+ "'=>Get the value from column=:"+ col + "and RowNo=: " + row + " : is "+ value);
											return value2;
										} else {
											extentTestReport.log(LogStatus.FAIL,"getCellData()", getVal + " is null");
											FreeCRMLogger.writeToLog("SeleniumKeywordslib","getCellData()", getVal + " is null");
										}
					...
					FreeCRMLogger.writeToLog("SeleniumKeywordslib","getCellData()", "'" + objName+ "' =>does not exist!");
					return null;
				}
			}catch (NoSuchElementException ns) {
				try
				{
				ns.printStackTrace();
				FreeCRMLogger.writeToLog("SeleniumKeywordslib", "getCellData()","Object not found exeption thrown");
				FreeCRMLogger.writeToLog("SeleniumKeywordslib", "getCellData()","'" + objName + "'object does not exist");
				return value;
				}
			 finally {
	       		 extentTestReport.addScreenCapture(propertyFileReader.getValue("REPORT_PATH")+"objName");
	 ...
	 public String getCellData(WebDriver webDriver, String objName,String selector,String selectorvalue, String rowName, String colName,ExtentReports extentReports,ExtentTest extentTestReport) {
			...
			try {
				...
				if (we != null) {
					...
					FreeCRMLogger.writeToLog("SeleniumKeywordslib","getRowByContent() ", "rows - " + rows.size());
					if (rows != null) {
						int tempRowNo = -1;
						for (WebElement werowCell : rows) {
							String value1 = null;
						...
						if (value1.contains(rowName)) {
							...
							if (value2 != null) {
								if (value2.equalsIgnoreCase(colName)) {
									colNo = tempColNo;
									FreeCRMLogger.writeToLog("SeleniumKeywordslib","getRowByContent() ","VALUES EQUAL");
									FreeCRMLogger.writeToLog("SeleniumKeywordslib","getRowByContent() ","Col number captured is = \""+ colNo + "\"");
									col = colNo;
									
									
									WebElement weCell = (we.findElements(By.tagName("tr"))).get(row).findElements(By.tagName("td")).get(col);
									FreeCRMLogger.writeToLog("SeleniumKeywordslib","getCellData()", "weCell - " + weCell);
									...
									if (weCell != null) {
										...
										extentTestReport.log(LogStatus.FAIL,"getCellData()", getVal + " is null");
										FreeCRMLogger.writeToLog("SeleniumKeywordslib","getCellData()", "value is null");
									}
								}
							}
								}
...
public boolean compareData(WebDriver webDriver,String Actualvalue, String Expectedvalue,String Match,ExtentReports extentReports,ExtentTest extentTestReport) {
	FreeCRMLogger.writeToLog("SeleniumKeywordslib", "compareData() ",
...
/*public String moveToElementbyText(WebDriver webDriver, String objName,ExtentReports extentReports,ExtentTest extentTestReport) {
	...
	try {
       ...
       selectorvalue = "//*[normalize-space(text())="+ "'"+selectorvalue +"'"+"]";
		List<WebElement> list = getElements(webDriver, objName, selector,selectorvalue);
		Actions act = new Actions(webDriver);
       	FreeCRMLogger.writeToLog("SeleniumKeywordslib", "moveToElementbyText() ","web elements size - " + list.size());
		if (list.size() > 1) {
			FreeCRMLogger.writeToLog("SeleniumKeywordslib","moveToElementbyText() ", "found more than one element");
			FreeCRMLogger.writeToLog("SeleniumKeywordslib","moveToElementbyText() ", "clicking 1st element");
           ...
           // if (element.isDisplayed()) {
			...
			//}
				...
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","moveToElementbyText ", "'" + objName+ "Object was not visible");
        ...
        ns.printStackTrace();
        FreeCRMLogger.writeToLog("SeleniumKeywordslib", "moveToElementbyText() ","Object not found exeption thrown");
        FreeCRMLogger.writeToLog("SeleniumKeywordslib", "moveToElementbyText() ", "'" + objName + "'object does not exist");
        status = "false";
 	...
 	{
        ex.printStackTrace();
        FreeCRMLogger.writeToLog("SeleniumKeywordslib", "moveToElementbyText() ","Exception occurred");
...
/*public String datePicker(WebDriver webDriver, String objName,String targetDate,ExtentReports extentReports,ExtentTest extentTestReport) {
	String status = "false";
	  final Map<String,Integer> MONTH_TO_CALENDAR_INDEX = new HashMap<String,Integer>();
       ...
       String[] parts = menuItems.split("-");
        ...
        String targetYear =parts[2];
          ...
          try{  
	  //  List<WebElement> list = getElements(webDriver, objName, selector,selectorvalue);
			FreeCRMLogger.writeToLog("SeleniumKeywordslib", "datePicker() ","web elements size - " + list.size());
			if (list.size() > 1) {
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","datePicker() ", "found more than one element");
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","datePicker() ", "clicking 1st element");
		            ...
		            for (WebElement we : tableOfDays.findElements(By.tagName("span"))) {
		               if (we.getText().contains((targetday))) {
							...
							FreeCRMLogger.writeToLog("SeleniumKeywordslib","datePicker()", "'"+ targetDate + "' =>Selected");
					}
				}
	      ...
	      //      WebElement picker = webDriver.findElement(By.xpath("//*[@src='../assets/images/calendar.svg']"));
				...
				extentTestReport.log(LogStatus.FAIL, "move to element" +objName);
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","datePicker ", "'" + objName+ "Object was not visible");
				status = "Link was not visible";
				return status;
			}
        ...
        ns.printStackTrace();
        FreeCRMLogger.writeToLog("SeleniumKeywordslib", "datePicker() ","Object not found exeption thrown");
        FreeCRMLogger.writeToLog("SeleniumKeywordslib", "datePicker() ", "'" + objName + "'object does not exist");
        status = "false";
 	...
 	{
        ex.printStackTrace();
        FreeCRMLogger.writeToLog("SeleniumKeywordslib", "datePicker() ","Exception occurred");
        extentTestReport.log(LogStatus.FAIL,"select " +objName+ "option");
        String err[]=ex.getMessage().split("\n");
		status = "Exception " +err[0].replaceAll("'", "") + " Exception Occurred";
		return status;
		...
		public String selectRadioButtonbyText(WebDriver webDriver, String objName,String selector,String selectorvalue,String radiobuttonName,ExtentReports extentReports,ExtentTest extentTestReport) {
			...
			try {
				...
				if(paramvalues.length==0)
				{
					...
					FreeCRMLogger.writeToLog("SeleniumKeywordslib","selectRadioButton() ", "'" + objName+ " RadioButton was not found");
					status = "CheckBox was not found";
				}
	            ...
	            ns.printStackTrace();
	            FreeCRMLogger.writeToLog("SeleniumKeywordslib", "selectRadioButton() ","Object not found exeption thrown");
	            FreeCRMLogger.writeToLog("SeleniumKeywordslib", "selectRadioButton() ", "'" + objName + "'object does not exist");
	            status = "false";
	            extentTestReport.log(LogStatus.FAIL, "Unable to select " +objName+ "Checkbox");
	            return status;
	     }   catch (Exception ex) {
					...
					if (list.size() > 1) {
						...
						for (WebElement element : list) {
							if (element.isDisplayed()) {
									...
									if (element.isSelected()) {
										FreeCRMLogger.writeToLog("SeleniumKeywordslib","setCheckbox() ", "'" + objName+ "' => selected/Holiday");
										extentTestReport.log(LogStatus.PASS, "File Name " +objName);
										return "true";
									} else {
										FreeCRMLogger.writeToLog("SeleniumKeywordslib","setCheckbox() ", "'" + objName+ "' => is  not Holiday");
										extentTestReport.log(LogStatus.PASS, "File Name " +objName);
										return "false";
							}
							}
...
public String getCellDataByRowNoAndColNo(WebDriver webDriver, String objName,String selector,String selectorvalue, String rowNo, String colNo,ExtentReports extentReports,ExtentTest extentTestReport) {
	FreeCRMLogger.writeToLog("SeleniumKeywordslib", "getCellDataByRowNoAndColNo()","getCellDataByRowNoAndColNo  rowNo " + rowNo + " colNo " + colNo+ "..objName - " + objName);
	String value = null;
	String getVal = null;
	...
	try {
		...
		rows = (we.findElements(By.xpath(".//tr")));
			...
			FreeCRMLogger.writeToLog("SeleniumKeywordslib","getCellDataByRowNoAndColNo() ", "rows - " + rows.size());
			if (rows != null) {
				...
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","getCellDataByRowNoAndColNo()", "weCell - " + weCell);
							...
							if (weCell != null) {
								...
								if (getVal != null) {
									...
									FreeCRMLogger.writeToLog("SeleniumKeywordslib","getCellDataByRowNoAndColNo()", "'" + objName+ "'=>Get the value from column=:"+ colNo + "and RowNo=: " + rowNo + " : is "+ value);
									return value;
								} else {
									//extentTestReport.log(LogStatus.FAIL,"getCellDataByRowNoAndColNo()", getVal + " is null");
									FreeCRMLogger.writeToLog("SeleniumKeywordslib","getCellDataByRowNoAndColNo()", getVal + " is null");
								}
							} else {
								extentTestReport.log(LogStatus.FAIL,"getCellDataByRowNoAndColNo()", getVal + " is null");
								FreeCRMLogger.writeToLog("SeleniumKeywordslib","getCellDataByRowNoAndColNo()", "value is null");
							}
						}
		  } 
		else {
			extentTestReport.log(LogStatus.FAIL,"getCellDataByRowNoAndColNo()", "'" + objName+ "' =>does not exist!");
			FreeCRMLogger.writeToLog("SeleniumKeywordslib","getCellDataByRowNoAndColNo()", "'" + objName+ "' =>does not exist!");
			return null;
		}
	}catch (NoSuchElementException ns) {
		try
		{
		ns.printStackTrace();
		FreeCRMLogger.writeToLog("SeleniumKeywordslib", "getCellDataByRowNoAndColNo()","Object not found exeption thrown");
		FreeCRMLogger.writeToLog("SeleniumKeywordslib", "getCellDataByRowNoAndColNo()","'" + objName + "'object does not exist");
		return value;
		}
	 finally {
   		 extentTestReport.addScreenCapture(propertyFileReader.getValue("REPORT_PATH")+"objName");
...
public HashMap<String, String> validateColumnDataInTable(WebDriver webDriver, String objName,String selector,String selectorvalue, String colName,String data,ExtentReports extentReports,ExtentTest extentTestReport) {
	FreeCRMLogger.writeToLog("SeleniumKeywordslib", "getCellData()","getCellData  rowNo " + " colNo " + colName+ "..objName - " + objName);
	String value = null;
	String getVal = null;
	int rowNo = 0;
	int colNo = 0;
	...
	try {
		...
		if (we != null) {
       	...
       	FreeCRMLogger.writeToLog("SeleniumKeywordslib","getRowByContent() ", "rows - " + rows.size());
			...
			FreeCRMLogger.writeToLog("SeleniumKeywordslib","getCellData()",  "X" + colName);
			value1 = werowCell.getText();
			FreeCRMLogger.writeToLog("SeleniumKeywordslib","getRowByContent() ", "VALUE - "+ value1);
	...
	String status = "false";
       ...
       try{  
	    List<WebElement> list1 = getElements(webDriver, objName, selector,selectorvalue);
			FreeCRMLogger.writeToLog("SeleniumKeywordslib", "setKeyDates() ","web elements size - " + list1.size());
			if (list1.size() == 1) {
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","setKeyDates() ", "found more than one element");
				 int rowNo = 0;
						int colNo = 0;
						int  row=0;
						int col=0;
				...
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","setKeyDates() ", "rows - " + rows.size()+" and row num" +row);
				if (rows != null) {
					int tempRowNo = -1;
					for (WebElement werowCell : rows) {
						String value1 = null;
						tempRowNo++;
				value1 = werowCell.getText();
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","setKeyDates() ", "VALUE - "+ value1);
				...
				if (value1 != null) {
						...
						for(int i=0;i<setKeyDates.length;i++)
					{
						...
						we = collist.get(0);
					 cols = (we.findElements(By.xpath(".//th")));
					  FreeCRMLogger.writeToLog("SeleniumKeywordslib","setKeyDates() ", "rows - " + cols.size());
			             ...
			             for(WebElement wecolCell :cols)
				             {
							    tempColNo++;
							     ...
							     if(value2 == null)
								   {
							    	 FreeCRMLogger.writeToLog("SeleniumKeywordslib","setKeyDates() ", "value2 null " + value2);
				...
				extentTestReport.log(LogStatus.FAIL, "move to element" +objName);
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","setKeyDates ", "'" + objName+ "Object was not visible");
				status = "Link was not visible";
				return status;
			}
        ...
        ns.printStackTrace();
        FreeCRMLogger.writeToLog("SeleniumKeywordslib", "setKeyDates() ","Object not found exeption thrown");
        FreeCRMLogger.writeToLog("SeleniumKeywordslib", "setKeyDates() ", "'" + objName + "'object does not exist");
        status = "false";
        extentTestReport.log(LogStatus.FAIL, "Unable to click on " +objName+ "Calender");
        return status;
 } catch (Exception ex) {
 	try
 	{
 		...
 		ex.printStackTrace();
        FreeCRMLogger.writeToLog("SeleniumKeywordslib", "setKeyDates() ","Exception occurred");
        extentTestReport.log(LogStatus.FAIL,"select " +objName+ "option");
        String err[]=ex.getMessage().split("\n");
		status = "Exception " +err[0].replaceAll("'", "") + " Exception Occurred";
		return status;
...
public String setProgramRoleForTeamMember(WebDriver webDriver, String objName,String rowName,String programRoles,ExtentReports extentReports,ExtentTest extentTestReport) {
	String status = "false";
      ...
      //  String[] prgramRoles= programRoles.split(";");
       ...
       try{  
	    List<WebElement> list1 = getElements(webDriver, objName, selector,selectorvalue);
			FreeCRMLogger.writeToLog("SeleniumKeywordslib", "setProgramRoleForTeamMember() ","web elements size - " + list1.size());
			if (list1.size() == 1) {
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","setProgramRoleForTeamMember() ", "found more than one element");
				 int rowNo = 0;
						int colNo = 0;
						int  row=0;
						int col=0;
				...
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","setProgramRoleForTeamMember() ", "rows - " + rows.size()+" and row num" +row);
				if (rows != null) {
					int tempRowNo = -1;
					for (WebElement werowCell : rows) {
						String value1 = null;
						tempRowNo++;
				value1 = werowCell.getText();
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","setProgramRoleForTeamMember() ", "VALUE - "+ value1);
				...
				if (value1 != null) {
						...
						for(int i=0;i<programRoles.length();i++)
					{
						...
						we = collist.get(0);
					 cols = (we.findElements(By.xpath(".//th")));
					  FreeCRMLogger.writeToLog("SeleniumKeywordslib","setProgramRoleForTeamMember() ", "rows - " + cols.size());
			             ...
			             for(WebElement wecolCell :cols)
				             {
							    tempColNo++;
							     ...
							     if(value2 == null)
								   {
							    	 FreeCRMLogger.writeToLog("SeleniumKeywordslib","setProgramRoleForTeamMember() ", "value2 null " + value2);
			...
			else if(list1.size()>1)
			{
			             ...
			             for(WebElement wecolCell :cols)
				             {
							    tempColNo++;
							     ...
							     if(value2 == null)
								   {
							    	 FreeCRMLogger.writeToLog("SeleniumKeywordslib","setProgramRoleForTeamMember() ", "value2 null " + value2 +selection +input);
							    									   }
								 else if (value2 != null) 
					    	       {
					    		   colNo = tempColNo;
				...
				extentTestReport.log(LogStatus.FAIL, "move to element" +objName);
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","setProgramRoleForTeamMember ", "'" + objName+ "Object was not visible");
				status = "Link was not visible";
				return status;
			}
        ...
        ns.printStackTrace();
        FreeCRMLogger.writeToLog("SeleniumKeywordslib", "setProgramRoleForTeamMember() ","Object not found exeption thrown");
        FreeCRMLogger.writeToLog("SeleniumKeywordslib", "setProgramRoleForTeamMember() ", "'" + objName + "'object does not exist");
        status = "false";
        extentTestReport.log(LogStatus.FAIL, "Unable to click on " +objName+ "Calender");
        return status;
 } catch (Exception ex) {
 	try
 	{
 		...
 		ex.printStackTrace();
        FreeCRMLogger.writeToLog("SeleniumKeywordslib", "setProgramRoleForTeamMember() ","Exception occurred");
        extentTestReport.log(LogStatus.FAIL,"select " +objName+ "option");
        String err[]=ex.getMessage().split("\n");
		status = "Exception " +err[0].replaceAll("'", "") + " Exception Occurred";
		return status;
 ...
 * '################################################################################################
 ...
 public String selectCategoryRadio(WebDriver webDriver, String objName,String selector,String selectorvalue,String text,ExtentReports extentReports,ExtentTest extentTestReport) {
	...
	public String checkElementDisableState(WebDriver webDriver, String objName,String selector,String selectorvalue,String disabledElements,ExtentReports extentReports,ExtentTest extentTestReport) {
		String status = "false";
		try {
			  	     FreeCRMLogger.writeToLog("SeleniumKeywordslib","checkElementDisableState() ", "clicking 1st element");
					  selectorvalue = "//*[text()="+ "'"+disabledElements +"'"+"]";
					  List<WebElement> list = getElements(webDriver, objName,selector,selectorvalue);
					  if (list.size() > 1) {
						FreeCRMLogger.writeToLog("SeleniumKeywordslib","checkElementDisableState() ", "found more than one element");
						FreeCRMLogger.writeToLog("SeleniumKeywordslib", "checkElementDisableState() ","web elements size - " + list.size());
					 // for (WebElement element : list) {
						WebElement element  =list.get(0);
				//	if (element.isDisplayed()) {
						
						if (element.getAttribute("class") != null&& element.getAttribute("class").contains("disabled")) {
								FreeCRMLogger.writeToLog("SeleniumKeywordslib","checkElementDisableState() ", "'" + objName+ "' => is disabled");
								extentTestReport.log(LogStatus.PASS, "File Name " +disabledElements+"   is  disabled");
								status=  "true";
								return status;
							} else {
								FreeCRMLogger.writeToLog("SeleniumKeywordslib","checkElementDisableState() ", "'" + objName+ "' => is  enabled");
						...
						if ((element.getAttribute("class") != null&& element.getAttribute("class").contains("disabled")) ||(element.getTagName().equals("button")) ||(element.getTagName().equals("label"))) {
									...
									if((element.getTagName().equals("button")) ||element.getTagName().equals("label"))
									{
										...
										{
											FreeCRMLogger.writeToLog("SeleniumKeywordslib","checkElementDisableState() ", "'" +disabledElements+ "' => is disabled");
...
//								}
								...
								FreeCRMLogger.writeToLog("SeleniumKeywordslib","checkElementDisableState() ", "'" + objName+ "CheckBox was not visible");
								status = "CheckBox was not visible";
								return status;
							}
	  		 ...
	  		 {
	  			    exception.printStackTrace();
	  				FreeCRMLogger.writeToLog("SeleniumKeywordslib","checkElementDisableState ", "Exeption thrown");
	  				FreeCRMLogger.writeToLog("SeleniumKeywordslib","checkElementDisableState ", "object doesnot exists");
					 ...
					 // for (WebElement element : list) {
						...
						if ((element.getAttribute("class") != null&& element.getAttribute("class").contains("disabled")) ||(element.getTagName().equals("input")) ||(element.getTagName().equals("label")))
						{
							if(element.getTagName().contains("checked"))
											{
										FreeCRMLogger.writeToLog("SeleniumKeywordslib","checkElementDisableState() ", "'" +element+ "' => is disabled");
									...
									{
									FreeCRMLogger.writeToLog("SeleniumKeywordslib","checkElementDisableState() ", "'" +element+ "' => is not checked");
				...
				//	if (element.isDisplayed()) {
						...
						if (element.getTagName().contains("checked")) {
							...
							WebElement element = list.get(0);
									...
									if(element.getTagName().contains("checked"))
									{
											FreeCRMLogger.writeToLog("SeleniumKeywordslib","checkElementDisableState() ", "'" +objName+ "' => is checked");
										...
										//	extentTestReport.log(LogStatus.PASS, " checkbox  is  checked");
											...
											{
											FreeCRMLogger.writeToLog("SeleniumKeywordslib","checkElementDisableState() ", "'" +objName+ "' => is not checked");
					  ...
					  {
						  FreeCRMLogger.writeToLog("SeleniumKeywordslib","checkElementDisableState() ", "'" +objName+ "' => is not visible");
...
//							
		...
		public String checkboxDisableState(WebDriver webDriver, String objName,String selector,String selectorvalue,String assertValue,ExtentReports extentReports,ExtentTest extentTestReport) {
			String status = "false";
			try {
				  	     FreeCRMLogger.writeToLog("SeleniumKeywordslib","checkboxDisableState() ", "clicking 1st element");
						 List<WebElement> list = getElements(webDriver, objName,selector,selectorvalue);
						  if (list.size() > 1) {
							FreeCRMLogger.writeToLog("SeleniumKeywordslib","checkboxDisableState() ", "found more than one element");
							FreeCRMLogger.writeToLog("SeleniumKeywordslib", "checkboxDisableState() ","web elements size - " + list.size());
							WebElement element =list.get(0);
							//for (WebElement element : list) {
						//if (element.isDisplayed()) {
							if(assertValue.contains("disabled")){
							if (element.getTagName().contains(assertValue)) {
									FreeCRMLogger.writeToLog(element +"   is  " +assertValue);
									extentTestReport.log(LogStatus.PASS,element +"   is  " +assertValue);
									status=  "true";
									return status;
								} else {
									FreeCRMLogger.writeToLog("SeleniumKeywordslib","checkboxDisableState() ", "'" + objName+ "' => is  enabled");
...
//						}
				...
				//} 
								...
								if (element.isDisplayed()) {
									...
									if ((element.getTagName().contains(assertValue)) || (element.getAttribute("class").contains(assertValue))) {
												FreeCRMLogger.writeToLog("SeleniumKeywordslib","checkboxDisableState() ", "'" +element+ "' => is disabled");
												extentTestReport.log(LogStatus.PASS,objName +"is "   +assertValue);
												status= "true";
												return status;
											} else {
									...
									else
									{
										...
										FreeCRMLogger.writeToLog("SeleniumKeywordslib","checkboxDisableState() ", "'" + objName+ " was not visible");
										status = "Element was not visible";
										return status;
									}
									...
									FreeCRMLogger.writeToLog("SeleniumKeywordslib","checkboxDisableState() ", "'" + objName+ "CheckBox was not visible");
									status = "CheckBox was not visible";
									return status;
								}
		  		 ...
		  		 {
		  			    exception.printStackTrace();
		  				FreeCRMLogger.writeToLog("SeleniumKeywordslib","checkboxDisableState ", "Exeption thrown");
		  				FreeCRMLogger.writeToLog("SeleniumKeywordslib","checkboxDisableState ", "object doesnot exists");
	...
	public String validateDropDownList(WebDriver webDriver,String objName, String selector,String selectorValue, String dropDownList,String assertValue,ExtentReports extentReports,ExtentTest extentTestReport) {
		FreeCRMLogger.writeToLog("SeleniumKeywordslib","validateDropDownList()", "Drop Down");
		String status = "false";
		try {
			List<WebElement> list = getElements(webDriver, objName, selector,selectorValue);
			...
			if (list != null) {
				...
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","validateDropDownList()", "List" + lw1);
			List<String> dropList = Arrays.asList(dropDownList.split(","));
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","validateDropDownList()", "dropList" + dropList);
				if (assertValue.equalsIgnoreCase("true")) {
					for (String dropDownValue : dropList) {
						if (lw1.contains(dropDownValue)) {
							FreeCRMLogger.writeToLog("SeleniumKeywordslib","validateDropDownList()","dropDownValue" + dropDownValue);
							FreeCRMLogger.writeToLog("SeleniumKeywordslib","validateDropDownList()","List Matches");
							extentTestReport.log(LogStatus.PASS,lw1 +"  Contains the dopdown Value");
							status = "true";
							// return status;
						} else {
							FreeCRMLogger.writeToLog("SeleniumKeywordslib","validateDropDownList()","List doesnt Matches");
	...
	public String clickEnter(WebDriver webDriver, String objName,String selector,String selectorValue,ExtentReports extentReports,ExtentTest extentTestReport) {
		String status = "false";
		try {
			FreeCRMLogger.writeToLog("seleniumKeywordslib","clickEnter() ", " objName - " + objName+ " selector - " + selector);
			FreeCRMLogger.writeToLog("seleniumKeywordslib","clickEnter() ", "before finding element");
			List<WebElement> list = getElements(webDriver, objName, selector,selectorValue);
			...
			if (element != null) {
				...
				FreeCRMLogger.writeToLog("seleniumKeywordslib","clickEnter() ", "after click on element");
			...
			e.printStackTrace();
			FreeCRMLogger.writeToLog("seleniumKeywordslib","clickEnter() ","Exception exeption thrown in clickEnter"+ e.getMessage());
			String err[]=e.getMessage().split("\n");
			status=  "Exception " +err[0].replaceAll("'", "") + " Occurred";
			return status;
		...
		String status = "false";
	       ...
	       try{  
		    List<WebElement> list1 = getElements(webDriver, objName, selector,"//*[@class='assessment-table']");
				FreeCRMLogger.writeToLog("SeleniumKeywordslib", "setScoreInAssessmentTab() ","web elements size - " + list1.size());
				if (list1.size() == 1) {
					FreeCRMLogger.writeToLog("SeleniumKeywordslib","setScoreInAssessmentTab() ", "found more than one element");
					 int  row=0;
					List<WebElement> list = getElements(webDriver, objName, selector,selectorvalue);
					WebElement we = null;
					if (list != null)
					...
					FreeCRMLogger.writeToLog("SeleniumKeywordslib","setScoreInAssessmentTab() ", "rows - " + rows.size()+" and row num" +row);
					if (rows != null) {
					//	int tempRowNo = -1;
						for (WebElement werowCell : rows) {
							String value1 = null;
						//	tempRowNo++;
					value1 = werowCell.getText();
					FreeCRMLogger.writeToLog("SeleniumKeywordslib","setScoreInAssessmentTab() ", "VALUE - "+ value1);
					...
					extentTestReport.log(LogStatus.FAIL, "move to element" +objName);
					FreeCRMLogger.writeToLog("SeleniumKeywordslib","setScoreInAssessmentTab ", "'" + objName+ "Object was not visible");
					status = "Link was not visible";
					return status;
				}
	        ...
	        ns.printStackTrace();
	        FreeCRMLogger.writeToLog("SeleniumKeywordslib", "setScoreInAssessmentTab() ","Object not found exeption thrown");
	        FreeCRMLogger.writeToLog("SeleniumKeywordslib", "setScoreInAssessmentTab() ", "'" + objName + "'object does not exist");
	        status = "false";
	...
	public String checkNoOfRecords(WebDriver webDriver,String objName, String selector,String selectorvalue,ExtentReports extentReports,ExtentTest extentTestReport) {
		FreeCRMLogger.writeToLog("SeleniumKeywordslib","checkNoOfRecords() ", " objName - "+ objName);
		String rowsize="";
		try {
			List<WebElement> list = getElements(webDriver, objName, selector,selectorvalue);
			WebElement we = null;
			...
			if (we != null)
			{
				...
				mRows = rows.size();
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","checkNoOfRecords() ","Total rows - " +( mRows-1));
				if(rows != null)
				{
					    rowsize = Integer.toString(rows.size());
					    extentTestReport.log(LogStatus.PASS, "No of rows in table" +rowsize);
						FreeCRMLogger.writeToLog("SeleniumKeywordslib","checkNoOfRecords() ", "No of rows in table matches with expected value");
						return rowsize;
				}
			 ...
			 {
				 extentTestReport.log(LogStatus.FAIL, "'"+ objName + "' =>does not exist!");
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","checkNoOfRecords() ", "'"+ objName + "' =>does not exist!");
				rowsize="0";
				return rowsize;
			 }
	  		 ...
	  		 {
	  			    exception.printStackTrace();
	  				FreeCRMLogger.writeToLog("SeleniumKeywordslib","checkNoOfRecords() ", "Unable to get num of rows in table");
	  		 }
	  		 finally{
	...
	public String setMetricsforGateways(WebDriver webDriver, String objName,String rowName,String keyDates,ExtentReports extentReports,ExtentTest extentTestReport) {
		String status = "false";
	       ...
	       try{  
		    List<WebElement> list1 = getElements(webDriver, objName, selector,selectorvalue);
				FreeCRMLogger.writeToLog("SeleniumKeywordslib", "setMetricsforGateways() ","web elements size - " + list1.size());
				if (list1.size()==1) {
					FreeCRMLogger.writeToLog("SeleniumKeywordslib","setMetricsforGateways() ", "found more than one element");
					 int rowNo = 0;
							int colNo = 0;
							int  row=0;
							int col=0;
					...
					FreeCRMLogger.writeToLog("SeleniumKeywordslib","setMetricsforGateways() ", "rows - " + rows.size()+" and row num" +row);
					if (rows != null) {
						int tempRowNo = -1;
						for (WebElement werowCell : rows) {
							String value1 = null;
							tempRowNo++;
					value1 = werowCell.getText();
					FreeCRMLogger.writeToLog("SeleniumKeywordslib","setMetricsforGateways() ", "VALUE - "+ value1);
					...
					if (value1 != null) {
							...
							for(int i=0;i<setMetricsforGateways.length;i++)
						{
							...
							we = collist.get(0);
						 cols = (we.findElements(By.xpath(".//th")));
						  FreeCRMLogger.writeToLog("SeleniumKeywordslib","setMetricsforGateways() ", "rows - " + cols.size());
				             ...
				             for(WebElement wecolCell :cols)
					             {
								    tempColNo++;
								     ...
								     if(value2 == null)
									   {
								    	 FreeCRMLogger.writeToLog("SeleniumKeywordslib","setMetricsforGateways() ", "value2 null " + value2);
					...
					extentTestReport.log(LogStatus.FAIL, "move to element" +objName);
					FreeCRMLogger.writeToLog("SeleniumKeywordslib","setMetricsforGateways ", "'" + objName+ "Object was not visible");
					status = "Link was not visible";
					return status;
				}
	        ...
	        ns.printStackTrace();
	        FreeCRMLogger.writeToLog("SeleniumKeywordslib", "setMetricsforGateways() ","Object not found exeption thrown");
	        FreeCRMLogger.writeToLog("SeleniumKeywordslib", "setMetricsforGateways() ", "'" + objName + "'object does not exist");
	        status = "false";
	        extentTestReport.log(LogStatus.FAIL, "Unable to click on " +objName+ "Calender");
	        return status;
	 } catch (Exception ex) {
	 	try
	 	{
	 		...
	 		ex.printStackTrace();
	        FreeCRMLogger.writeToLog("SeleniumKeywordslib", "setMetricsforGateways() ","Exception occurred");
	        extentTestReport.log(LogStatus.FAIL,"select " +objName+ "option");
	        String err[]=ex.getMessage().split("\n");
			status = "Exception " +err[0].replaceAll("'", "") + " Exception Occurred";
			return status;
   ...
   * '################################################################################################
	  ...
	  public String checkLengthofString(WebDriver webDriver,String objName,String selectorvalue,ExtentReports extentReports,ExtentTest extentTestReport) {
			...
			if(len <=100) 
			{
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","checkLengthofString()","'"+  "' =>Length of  "+ objName +":"+len);
	   ...
	   * '################################################################################################
		  ...
		  public String compareRetainTillDateis30DaysAfterCurrentDate(WebDriver webDriver,String objName,String visibleDate,ExtentReports extentReports,ExtentTest extentTestReport) {
				...
				{
					FreeCRMLogger.writeToLog("SeleniumKeywordslib","compareRetainTillDateis30DaysAfterCurrentDate()","'"+  "' =>Length of  "+ objName +":");
		  ...
		  public HashMap<String, String> selectSalesModelsFromSerialNumbersTab(WebDriver webDriver,String objName, String selector,String selectorValue,String salesModelType,ExtentReports extentReports,ExtentTest extentTestReport) {
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","selectSalesModelsFromSerialNumbersTab()", "Drop Down");
				 HashMap<String, String> map = new HashMap<String, String>();
				try {
					List<WebElement> list = getElements(webDriver, objName, selector,selectorValue);
					WebElement selectElement = null;
					if (list != null) {
						...
						for (WebElement webElement : s.getOptions()) {
							if(webElement.getText().contains(salesModelType))
							{
								...
								FreeCRMLogger.writeToLog("SeleniumKeywordslib","selectSalesModelsFromSerialNumbersTab()", "List" + map);
   ...
   * * ##########################################################################
	 ...
	 * '################################################################################################
		public String selectMultipleOptionsFromList(WebDriver webDriver, String objName,String selector, String selectorvalue,String paramvalues, String selection,ExtentReports extentReports,ExtentTest extentTestReport) {
				FreeCRMLogger.writeToLog("SeleniumKeywordslib", "select FromList() "," objName - " + objName + ".. input -.. selection - " + selection);
				String status = "false";
						...
						if (selectElement != null) {
							if (selectElement.isDisplayed()) {
								...
								if ((selectElement.getTagName().equalsIgnoreCase("input"))|| (selectElement.getTagName().equalsIgnoreCase("span"))) {
									...
									selectElement.sendKeys(Keys.RETURN);
									FreeCRMLogger.writeToLog("SeleniumKeywordslib","select FromList() ",input[i]+ "=> Item was selected successfully from the"+ "'" + objName + "'"+ "ListBox");
									status = "true";
   ...
   * * ##########################################################################
	 ...
	 * '################################################################################################	
		public String validateTableData(WebDriver webDriver, String objName,String selector,String selectorvalue,String data,ExtentReports extentReports,ExtentTest extentTestReport) {
			FreeCRMLogger.writeToLog("SeleniumKeywordslib", "validateTableData()","validateTableData  " + data+ "..objName - " + objName);
			String value = null;
			String getVal = null;
		    String status ="";
			int  row=0;
			try {
		       ...
		       FreeCRMLogger.writeToLog("SeleniumKeywordslib","validateTableData() ", "rows - " + rows.size());
					if (rows != null) {
						int tempRowNo = 0;
						for (WebElement werowCell : rows) {
							String value1 = null;
					...
					FreeCRMLogger.writeToLog("SeleniumKeywordslib","validateTableData()",  "X" + tempRowNo);
					value1 = werowCell.getText();
					FreeCRMLogger.writeToLog("SeleniumKeywordslib","validateTableData() ", "VALUE - "+ value1);
					if ((value1 != null) && (value1.contains(data))) 
					{
					     extentTestReport.log(LogStatus.PASS," Value  in RowName : " + " : is "+ value);
						 FreeCRMLogger.writeToLog("SeleniumKeywordslib","getCellData()", "'" + objName+ "'=>Get the value from column=:"+ data + "and RowNo=: " + row + " : is "+ value);
						 status= "true";
						 return status;
					}
					else
	...
	/*	public String selectFrame(WebDriver webDriver, String objName,String selector,String selectorValue) {
			...
			try {
               try {
				...
				ns.printStackTrace();
				FreeCRMLogger.writeToLog("SeleniumKeywordslib", "selectFrame() ","Object not found exeption thrown");
				FreeCRMLogger.writeToLog("SeleniumKeywordslib", "selectFrame() ","'" + objName + "'object does not exist");
				status = "" + objName + "object does not exist";
				return status;
			} catch (Exception ex) {
				ex.printStackTrace();
				FreeCRMLogger.writeToLog("SeleniumKeywordslib", "selectFrame() ","Exception occurred");

				FreeCRMLogger.writeToLog("SeleniumKeywordslib", "selectFrame() ","Exception " + ex.getMessage() + " Occurred");
				String err[]=ex.getMessage().split("\n");
				status =  "Exception " +err[0].replaceAll("'", "") + " Occurred";
				return status;
			}
		...
		public String switchToDefaultContent(WebDriver webDriver) {
			...
			try {
				webDriver.switchTo().defaultContent();
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","switchToDefaultContent() ","Switched to default content");
				return "true";
			}

			catch (Exception e) {
				e.printStackTrace();
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","switchToDefaultContent() ","Exception exeption thrown in Html_OpenURL"+ e.getMessage());
				String err[]=e.getMessage().split("\n");
				status=  "Exception " +err[0].replaceAll("'", "") + " Occurred";
				return status;
		...
		public String waitForPageLoaded(WebDriver webDriver) {
			...
			try {
				...
				ns.printStackTrace();
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","waitForPageLoaded()","Object not found exeption thrown");
				status = "Object not found exception was thrown";
				return status;
			} catch (Exception ex) {
				ex.printStackTrace();
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","waitForPageLoaded()", "Exception occurred");
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","waitForPageLoaded()", "Exception '" + ex.getMessage()+ "' Occurred");
				String err[]=ex.getMessage().split("\n");
				status=  "Exception " +err[0].replaceAll("'", "") + " Occurred";
				return status;
			}
			...
			//	 String status="";
				try
				{
					WebElement we = null;
					  ...
					  FreeCRMLogger.writeToLog("SeleniumKeywordslib","getTableHeaders() ", "rows - " + cols.size());
		  ...
		  public String verifyListOfDeliverables(WebDriver webDriver, String objName,String selector,String selectoValue,String listofDeliverablesinTeamTab,ExtentReports extentReports,ExtentTest extentTestReport) throws Exception {
				...
				try
				{
					WebElement we = null;
						...
						we = collist.get(0);
					   cols = (we.findElements(By.xpath(selectoValue)));
					   FreeCRMLogger.writeToLog("SeleniumKeywordslib","verifyListOfDeliverables() ", "rows - " + cols.size());
	          		...
	          		for(int i=0;i<delivearbls.length;i++)
	          		{
	          			  ...
	          			  {
	          				FreeCRMLogger.writeToLog("SeleniumKeywordslib","verifyListOfDeliverables() ","element is  not displayed");
	        				status = "true";
		  ...
		  public String dragAndDrop(WebDriver webDriver, String objName,String selector,String sourceselectoValue,String desselectoValue,ExtentReports extentReports,ExtentTest extentTestReport) throws Exception {
				...
				try
				{
					  ...
					  FreeCRMLogger.writeToLog("SeleniumKeywordslib","verifyListOfDeliverables() ", "");
...
//				webDriver.get(URL);
			...
			private String fileUploadExe(String browserType,String objName) {
				...
				FreeCRMLogger.writeToLog("getAuthenticateExe - browserType "+browserType+"..");
				...
				FreeCRMLogger.writeToLog("exeName - "+exeName);
				return exeName;
			}
			
			public Boolean upload(String autoItExeName) {
				FreeCRMLogger.writeToLog("KeyWordActions - autoItExeName - "+autoItExeName);
...
/*public String selectMultilpleCheckboxesbyObj(WebDriver webDriver, String objName,ExtentReports extentReports,ExtentTest extentTestReport) {
	...
	try {
		...
		waitForExistence(webDriver, objName, selector,selectorvalue);
		List<WebElement> list = getElements(webDriver, objName, selector,selectorvalue);
		FreeCRMLogger.writeToLog("SeleniumKeywordslib", "selectCheckbox() ","web elements size - " + list.size());
		if (list.size() > 1) {
			FreeCRMLogger.writeToLog("SeleniumKeywordslib","selectCheckbox() ", "found more than one element");
			FreeCRMLogger.writeToLog("SeleniumKeywordslib","selectCheckbox() ", "clicking 1st element");
			//for (WebElement element : list) {
			WebElement element = list.get(0);
				...
				if (element.isDisplayed() ) {
					if (selValue.equalsIgnoreCase("true")) {
						if (element.isSelected()) {
							extentTestReport.log(LogStatus.PASS, "click on "+paramvalues[i] +" Checkbox");
							FreeCRMLogger.writeToLog("SeleniumKeywordslib","selectCheckbox() ", "'" + paramvalues[i]+ "' => checkbox is already selected");
							status ="true";
							//return "true";
						} else {
							element.click();
				...
				{
					FreeCRMLogger.writeToLog("SeleniumKeywordslib","selectCheckbox() ", "'" + objName+ "is not checkbox");
					status = "CheckBox was not visible";
				}
			//}
		} 
	...
	List<WebElement> noofele = new ArrayList<WebElement>();
	  ...
	  try{  
	    List<WebElement> list1 = getElements(webDriver, objName, selector,selectorvalue);
	   	FreeCRMLogger.writeToLog("SeleniumKeywordslib", "setProgramRoleForTeamMember() ","web elements size - " + list1.size());
			if (list1.size() > 1) {
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","setProgramRoleForTeamMember() ", "found more than one element"+status);
				List<WebElement> list = getElements(webDriver, objName, selector,selectorvalue);
...
/*public boolean verifyAttributesSortedorNot(WebDriver webDriver, String objName,ExtentReports extentReports,ExtentTest extentTestReport) {
	...
	List<WebElement> noofele = new ArrayList<WebElement>();
	 boolean isSorted=true;
	  ...
	  try{  
	    List<WebElement> list1 = getElements(webDriver, objName, selector,selectorvalue);
	   	FreeCRMLogger.writeToLog("SeleniumKeywordslib", "setProgramRoleForTeamMember() ","web elements size - " + list1.size()+status);
			if (list1.size() > 1) {
				FreeCRMLogger.writeToLog("SeleniumKeywordslib","setProgramRoleForTeamMember() ", "found more than one element");
				List<WebElement> list = getElements(webDriver, objName, selector,selectorvalue);
					for (WebElement werowCell : list) {
...
public String DoubleClick(WebDriver webDriver, String objName,String selector,String selectorvalue,ExtentReports extentReports,ExtentTest extentTestReport) {
	...
	try {
		List<WebElement> list = getElements(webDriver, objName, selector,selectorvalue);
		FreeCRMLogger.writeToLog("SeleniumKeywordslib", "DoubleClick()",
		...
		if (list.size() > 1) {
			...
			for (WebElement btnField : list) {
				if (btnField.isDisplayed()) {
					...
					FreeCRMLogger.writeToLog("SeleniumKeywordslib","DoubleClick()", "after click");
					status = "true";
					FreeCRMLogger.writeToLog("SeleniumKeywordslib","ButtonClick()","'"+ objName+ "' => BUTTON was successfully clicked");
					return status;
				}

				else {
					FreeCRMLogger.writeToLog("SeleniumKeywordslib","DoubleClick()", "'" + objName+ "Button was not visible");
					act.doubleClick(btnField).build().perform();
					status = "true";
					return status;
				}
			...
			WebElement btnField = list.get(0);
				...
				FreeCRMLogger.writeToLog("SeleniumKeywordslib", "DoubleClick()","'" + objName + "' =>does not exist!");
